...
// Based on: https://github.com/gauravahuja/Fanorona_AI/blob/master/Fanorona/Stone.h

#ifndef FANORONA_SERVER_STONE_H
#define FANORONA_SERVER_STONE_H
...
bool LegalMove(_Stone Stone, _WorldBoard Board, int NewX, int NewY);
...
int CanMakeCapturingMove(_Stone Stone, _WorldBoard Board, int &X, int &Y, int &Action);
...
int CaptureStones(_Stone &Stone, _WorldBoard &Board, int NewX, int NewY, int Action, bool real);

//Returns the list of Stones that can be captured by Approaching the point NewX,NewY
list<_Point2D> Approach(_Stone Stone, _WorldBoard Board, int NewX, int NewY);

//Returns the list of Stones that can be captured by Withdrawing to the point NewX,NewY
list<_Point2D> Withdraw(_Stone Stone, _WorldBoard Board, int NewX, int NewY);

//Returns true if the stone can Make Legal moves from its current postion
bool CanMove(_Stone Stone, _WorldBoard Board, int *x = NULL, int *y = NULL);
...
int MakeMove(_Stone &Stone, _WorldBoard &Board, int NewX, int NewY, bool real, int &NB);

//Selects the Stone on the Board and Unselects others Selected stones if any
void Select(_Stone Stone, _WorldBoard &Board);

//Returns true if the stone is selected on the board
bool IsSelected(_Stone Stone, _WorldBoard Board);

//Unselects the Stone on the Board
void UnSelect(_Stone Stone, _WorldBoard &Board);

//Returns true if the Stone can be captured by the opponent given the current board status
bool CanBeCaptured(_Stone Stone, _WorldBoard Board);

//Returns true if the Stone is on a Strong Position else false
bool IsOnStrongPosition(_Stone Stone);

#endif //FANORONA_SERVER_STONE_H
